...
from Models import InformativeEmission, WrightFisherDiffusion, _TIME_ROUNDING_ACCURACY
from extended_wf_model import ConditionalWrightFisherDisffusion
from pgas_dir import OutterPGAS, GP_sampler, SUniform_Sampler, MH_Sampler, RandomWalk_Proposal
from scalable_computing import Sample_processor, Particle_processor, Experiment
from pgas import BlockedParticleBridgeKernel, BlockedPGAS
from Utilities import MCMC_loader, TimeSeriesDataUtility
...
class ConditionalBayesianLearning(BayesianLearningExp):
    ...
    def _load_data(self, resume):
        ...
        try:
            ...
            self.dat = TimeSeriesDataUtility.read_time_series(
                self.original_data)

        except AttributeError:
            self.original_data = os.path.join(self.out_path, 'sample_data.tsv')
            self.full_original_data = os.path.join(
                self.out_path, 'sample_data_full.tsv')
            ...
            if resume:
                ...
                self.dat, self.full_dat = WrightFisherDiffusion.generate_full_sample_data(
                    silent=True, h=self.h, selectionCoefficients=self.s, obs_num=self.obs_num, end_time=self.end_time, epsilon=self.infer_epsilon, Ne=self.Ne, x0=self.true_x0, K=self.K_prime)
                TimeSeriesDataUtility.save_time_series(
                    self.dat, self.original_data)
                TimeSeriesDataUtility.save_time_series(
                    self.full_dat, self.full_original_data)

        if resume:
            self.mcmc_loader = MCMC_loader(self.out_path)
        # Filter the data for inference
        self.dat_inference = self.dat.loc[self.dat.time <= self.learn_time, [
            'time', 'K', 'X']]
        ...
        self.obs_inference = TimeSeriesDataUtility.tall_to_TK(
            self.dat_inference)
    ...
    def _test_xprime(self):
        g = InformativeEmission(
            alpha=[1]*self.K_prime, epsilon=self.infer_epsilon, b=self.infer_epsilon_tolerance)
        f = ConditionalWrightFisherDisffusion(
            h=self.h, K_prime=self.K_prime, K=self.K, Ne=self.Ne)
        gp_sampler = GP_sampler(dat=self.obs_inference['value'], obsTimes=self.time_points_inference,
                                epsilon=self.gp_epsilon, nOptRestarts=self.gp_n_opt_restarts, h=self.h)
        pg_bridge_kernel = BlockedParticleBridgeKernel(
            K=self.K, N=self.pgas_n_particles, proposalDistribution=gp_sampler, transitionDistribution=f, n_cores=self.bridge_n_cores)
        pgas_sampler = BlockedPGAS(bridgeKernel=pg_bridge_kernel, emissionDistribution=g, proposalDistribution=g, K_prime=self.K_prime, K=self.K, N=self.pgas_n_particles, T=len(
            self.time_points_inference), observations=self.obs_inference, transitionDistribution=f, h=self.h, rejuvenaion_prob=self.rejuvenaion_prob, n_cores=self.n_cores)
        xprime = pgas_sampler.generate_dummy_trajectory(self.learn_time)
        self.T = self.obs_inference['times'].shape[0]
        deltaT = self.obs_inference['times'][self.T -
                                             1] - self.obs_inference['times'][0]
        tau = int((round(deltaT/self.h, _TIME_ROUNDING_ACCURACY)))
        x_full_path = gp_sampler.sample_full_path(
            xprime[0, :], self.h, deltaT, tau)
    ...
    def _infer(self, resume):
        ...
        f = ConditionalWrightFisherDisffusion(
            h=self.h, K_prime=self.K_prime, K=self.K, Ne=self.Ne)
        gp_sampler = GP_sampler(dat=self.obs_inference['value'], obsTimes=self.time_points_inference,
                                epsilon=self.gp_epsilon, nOptRestarts=self.gp_n_opt_restarts, h=self.h, full_original_dat=None)
        pg_bridge_kernel = BlockedParticleBridgeKernel(
            K=self.K, N=self.pgas_n_particles, proposalDistribution=gp_sampler, transitionDistribution=f, n_cores=self.bridge_n_cores)
        pgas_sampler = BlockedPGAS(bridgeKernel=pg_bridge_kernel, emissionDistribution=g, proposalDistribution=g, K_prime=self.K_prime, K=self.K, N=self.pgas_n_particles, T=len(
            self.time_points_inference), observations=self.obs_inference, transitionDistribution=f, h=self.h, rejuvenaion_prob=self.rejuvenaion_prob, n_cores=self.n_cores)
        pgas_sampler.particle_processor = Particle_processor(
            time_points=self.time_points_inference, owner=self)

        xprime = pgas_sampler.generate_dummy_trajectory(self.learn_time)
        theta_init_sampler = SUniform_Sampler(
            dim_max=.5, dim_min=self.lower_s, K=self.K_prime)
        adapted_theta_proposal = RandomWalk_Proposal(mu=np.array([0]*self.K_prime), sigma=np.array(
            self.proposal_step_sigma), lower_bound=np.array([self.lower_s]*self.K_prime), upper_bound=np.array([self.upper_s]*self.K_prime))
        theta_sampler = MH_Sampler(
            adapted_proposal_distribution=adapted_theta_proposal, likelihood_distribution=f)

        fitness_learner = OutterPGAS(initial_distribution_theta=theta_init_sampler, initial_distribution_x=gp_sampler, observations=self.obs_inference,
                                     smoothing_kernel=pgas_sampler, parameter_proposal_kernel=theta_sampler, h=self.h, MCMC_in_Gibbs_nIter=self.MCMC_in_Gibbs_nIter)

        fitness_learner.sample_processor = Sample_processor(
            self.time_points_inference, self)

        is_resume = resume and os.path.isfile(
            '{}.gz'.format(self.inference_x_file_path))
        if is_resume:
    ...
    def _predict(self, xprime, theta_vector, resume):
        ...
        pg_bridge_kernel = BlockedParticleBridgeKernel(
            K=self.K, N=self.pf_n_particles, proposalDistribution=gp_sampler, transitionDistribution=f, n_cores=self.bridge_n_cores)
        pgas_sampler = BlockedPGAS(bridgeKernel=pg_bridge_kernel, emissionDistribution=g, proposalDistribution=g, K_prime=self.K_prime, K=self.K, N=self.pf_n_particles, T=T,
                                   observations=obs, transitionDistribution=f, h=self.h, rejuvenaion_prob=self.rejuvenaion_prob, n_cores=self.n_cores, disable_ancestor_bridge=self.disable_ancestor_bridge)
        pgas_sampler.particle_processor = Particle_processor(
            time_points=time_points, owner=self)

        fitness_learner = OutterPGAS(initial_distribution_theta=None, initial_distribution_x=gp_sampler,
                                     observations=obs, smoothing_kernel=pgas_sampler, parameter_proposal_kernel=None, h=self.h)
        ...
        if resume and os.path.isfile('{}.gz'.format(self.inference_x_file_path)):
            fitness_learner.data_loader = self.mcmc_loader
        is_resume = resume and os.path.isfile(
            '{}.gz'.format(self.prediction_file_path))
        xprime = pgas_sampler.generate_dummy_trajectory(self.learn_time)
        list_x_predict = fitness_learner.predict(
            t_learn=self.learn_time, theta_vector=filtered_res_theta, x0=xprime, is_resume=is_resume)
...
class CondExp(ConditionalBayesianLearning):
    ...
    def _easy_submit(batch_name, mem_per_job=50, n_jobs=None, time_per_job=80*60, ncores=1, k=2, seed=2):
        ...
        if n_jobs is None:
            n_jobs = int(n_params)
        batch_params = Experiment.get_batch_params_dict(
            batch_name=batch_name, mem_per_job=mem_per_job, n_jobs=n_jobs, time_per_job=time_per_job, ncpus_per_job=ncores)
        expt.submit_batch(batch_params=batch_params)

    def get_params(self):
        params = {}
        # set one less than the nonminal K (e.g. K=2 means 3 alleles )
        params['K_prime'] = [self.the_k]

        temp_k = params['K_prime'][0]

        params['s'] = [np.random.uniform(-.5, .5, temp_k).tolist()]
        params['true_x0'] = [(np.random.dirichlet(
            [1]*(temp_k+1), 1)[0][0:temp_k]).tolist()]
...
def run_model_comparison_ne(options, batch_path):
    ...
    expt.the_original_data = '{}_{}.tsv'.format(
        options['the_original_data'], options['the_Ne'])

    # Add the_h_config
    options['the_h_config'] = os.path.join(os.path.dirname(
        options['the_original_data']), 'h_config.tsv')
    expt.the_h = read_h_for_Ne(options['the_h_config'], expt.the_original_data)
    end_point, pred_point, expt.the_k, expt.the_obs_num = read_last_time_point(
        expt.the_original_data, int(options['the_do_predict']))
