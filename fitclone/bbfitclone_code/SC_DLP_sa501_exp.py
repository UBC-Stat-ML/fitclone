...
class Bayesian_learning_exp(Experiment):
    ...
    def _infer(self, resume):
        ...
        print('xprime...')
        # upper_s = .5 # up to August 5th, 2018
        upper_s = 10
        #lower_s = -0.99
        lower_s = -10
...
class TimingExp(Bayesian_learning_exp):
    ...
    def get_params(self):
        ...
        params['MCMC_in_Gibbs_nIter'] = [20] # Working on SA609
        #params['MCMC_in_Gibbs_nIter'] = [30]
        #params['out_path'] = ['~/Desktop/sc_dlp_sa609/exp']
    ...
    def logic(self, resume):
        ...
        last_theta, last_x = self._infer(resume)        
        res_theta = TimeSeriesDataUtility.read_time_series(self.inference_theta_file_path).values
        #res_theta = TimeSeriesDataUtility.read_time_series('/Users/sohrabsalehi/projects/fitness/batch_runs/SA609_test_counts_201810-22-22533.849597/outputs/o_0_0_9V7NA_201810-22-22540.709293/infer_theta.tsv.gz').values
        self._predict(xprime=None, theta_vector=res_theta, resume=resume)
...
# expt = TimingExp(); expt.the_k = 5; expt.ncpus_per_job = 1; tt = expt._run_default() # SA609
#expt = TimingExp(); expt.the_k = 5; expt.ncpus_per_job = 1; tt = expt._run_default() # SA532
...
def run_model_comparison_ne(options, batch_path):
    ...
    expt.the_original_data = '{}_{}.tsv'.format(options['the_original_data'], options['the_Ne'])
    #expt.the_h = read_h_for_Ne(options['the_h_config'], options['the_Ne'])
