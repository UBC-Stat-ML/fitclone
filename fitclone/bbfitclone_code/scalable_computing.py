...
# Todo: implement Python one to inherit from this one
...
# #    def process_particles(self, particle_time_list):
# #        the_len = len(particle_time_list)
# #        dfs = [None]*the_len
# #        for i in range(the_len):
#             if (particle_time_list[i] is None):
#                 next
#             fine_grained_times = np.linspace(self.time_points[i], self.time_points[i+1], num=particle_time_list[i].shape[1])
#             x_ar = xr.DataArray(particle_time_list[i], dims=self.dims, name='X', coords={'time':fine_grained_times})
#             df = x_ar.to_dataframe()
#             dfs[i] = df
#         res = pn.concat(dfs)
#         res['np'] = self.iter_index
#         if hasattr(self.owner, 'M_data'):
#             TimeSeriesDataUtility.save_time_series(res, self.owner.inference_particle_file_path[self.m_id], use_header=(self.iter_index==0))
#         else:
#             TimeSeriesDataUtility.save_time_series(res, self.owner.inference_particle_file_path, use_header=(self.iter_index==0))
#         self.iter_index = self.iter_index + 1
...
class Experiment(Configurable):
    ...
    def _profile_run_func(self):
        import profile
        import pstats
        #See more on profiling at https://docs.python.org/3/library/profile.html
        #res = profile.run('{}()._run_default()'.format(self.get_class_name()), '{}_{}_profile_stats'.format(self.get_class_name(), self.t))
        file_name = '{}_profile_stats'.format(self.get_class_name())
        res = profile.run('{}()._run_default()'.format(self.get_class_name()), file_name)
        #p = pstats.Stats('test_SmallPredictionExp_stats')
...
class Toy_exp(Experiment):
    ...
    def get_dependencies(self):
        #return(['pgas-dir.py', 'scalable_computing.py', 'data_generator.py'])
...
# from mvn_log_pdf import integrate_f
...
# 
# exp_path = os.path.expanduser('~/Desktop/pgas_sanity_check/exp_PJJKR_201707-02-165833.314854')
# s = MCMC_loader.get_all_theta(exp_path)
# X = MCMC_loader.get_all_x(exp_path, is_predict=False)
# from mvn_log_pdf import trivial_compute_loglikelihood
# trivial_compute_loglikelihood(x=X[1, :,:], s=s[1,:], h=0.001)
...
# from mvn_log_pdf import numpy_compute_loglikelihood
# numpy_compute_loglikelihood(x=X[1, :,:], s=s[1,:], h=0.001)
