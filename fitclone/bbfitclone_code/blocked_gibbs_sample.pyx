...
#cdef double _NUMERICAL_ACCURACY = 1e-10
...
#cdef void compute_sigma_bar(int K, double q, double[:] x, double[:,:] sigma2_bar):
    #print('input to compute_sigma_bar is ', np.asarray(x), q, K)
    cdef int k, j
    for k in range(K):
        for j in range(k+1):
            if k == j:
...
# @cython.nonecheck(False)
# @cython.cdivision(True)   
# cdef void sample_multivariate_normal(double [:] mean, double[:,:] cov, np.ndarray[DTYPE_t, ndim=1] x_out) nogil:
#     K = mean.shape[0]
#     with gil:
#         x_out[:] = sp.stats.multivariate_normal.rvs(mean=mean, cov=cov)    
...
cdef void propagate_with_noise(int K, int K_prime, double sqrt_h, double [:] mean, double[:,:] cov, double[:] normals, double[:,:] L, double[:] x_out, double[:] full_Xi) nogil:
    ...
    cdef int any_zero = 0
    #If the sum has reached one, use ridge correction on the covariance matrix to ameloriate the condition
    #for k in range(K+K_prime):
    #    the_sum = the_sum + full_Xi[k]
    #    if full_Xi[k] < _NUMERICAL_ACCURACY:
    #        any_zero = any_zero + 1
