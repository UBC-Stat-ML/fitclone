...
class ParticleGibbs(object):
    ...
    def _t_to_tau(self, t):
        # quick hack
        if (t == 0):
            return(int(round(self.obs['times'][t]/self.h, _TIME_ROUNDING_ACCURACY)))
        i = 0
...
class PGAS(ParticleGibbs):
    ...
    def generate_dummy_trajectory(self, T_learn):
        ...
        for t in range(xdummy.shape[0]):
            xdummy[t, ] = self.g.sample_posterior(learn_obs[t, ])
        #print('generate_dummy_trajectory_first: {}'.format(xdummy[0, ]))
        return(xdummy)
    
    # Modified for debugging purposes
    def sample_non_parallel_11(self, ReferenceTrajectory, theta, T_learn=None):
        ...
        self.x[0:(self.N-1), 0, :] = .5
        self.x[self.N-1, 0, ] = xprime[self._t_to_tau(0), ]
        
        llhood_weights = np.zeros([self.N])
        self.g.compute_loglikelihood_parallel(X=self.x[:, 0, :], y=self.obs['value'][0,], loglikelihood=llhood_weights, n_cores=self.n_cores)
        ...
        for t in range(1, self.T):
            ...
            #self.a[0:(self.N-1), t] = range(self.N-1)
            
            # Propagate particles
            the_slice = self._Xi_slice(t-1)
#             self.f.sample_parallel(X0=self.x[self.a[0:(self.N-1), t], t-1, ], 
...
#                                    x_out=self.x[0:(self.N-1), t, :],  n_threads=self.n_cores)
            ...
            if is_predicting is False and np.random.uniform(0, 1, 1) < self.rejuvenaion_prob and self.disable_ancestor_bridge is False:
                self.a[self.N-1, t], xprime[self._Xi_slice(t-1), ] = self.bridge.sample_joint_path_ancestor_not_parallel(particles=self.x[:,t-1,], initial_weights=self.w[:, t-1], time_length=deltaT, xprime=xprime[self._t_to_tau(t),], theta=self.theta)    
            else:
            ...
            if is_predicting == True:
                ...
                llhood_weights = np.zeros([self.N])
                self.g.compute_loglikelihood_parallel(X=self.x[:, t, :], y=self.obs['value'][t,], loglikelihood=llhood_weights, n_cores=self.n_cores)
        ...
        for k in range(self.N):
            ...
            #print(b_final, ' ==? ', self.a[k,1])
            # TODO: CHECK IF x_final is correct
            # Construct the full path 
            for i in range(self.T):
                #print('i is ', i, ' from self.T = ', self.T)
...
#                 x_full_final[self._t_to_tau(i), ] = x_final[i]
        ...
        #raise ValueError('STOP B')
        #TimeSeriesDataUtility.save_NTK(arr=my_x, output_path='/Users/sohrabsalehi/Desktop/skew_test/pgas_particles_sowed_3.tsv')
...
class BlockedPGAS(PGAS):
    ...
    def sample_non_parallel(self, xprime_full_path, theta, T_learn=None, shuffle_map=None):
        ...
        self.x[0:(self.N-1), 0, :] = self.r.sample_posterior_vectorised(observation=self.obs['value'][0, 0:self.K], N=self.N-1, free_freq=1-np.sum(A[0, :])) # HAS TO BE A[0,:], since it's initialisation with observations
        self.x[self.N-1, 0, ] = xprime_full_path[self._t_to_tau(0), 0:self.K]

        llhood_weights = np.zeros([self.N])
        self.g.compute_loglikelihood_parallel(X=self.x[:, 0, :], y=self.obs['value'][0, 0:self.K], loglikelihood=llhood_weights, n_cores=self.n_cores)
        self.w[:, 0] = np.exp(llhood_weights)
        ...
        for t in range(1, self.T):
            ...
            self.f.sample_parallel(X0=self.x[self.a[0:(self.N-1), t], t-1, ], Xi=self.Xi[0:(self.N-1), the_slice.start:the_slice.stop,:], x_out=self.x[0:(self.N-1), t, :], A=A[(the_slice.start-1):(the_slice.stop+1), :], s=self.theta, time_length=deltaT, seed=1, n_threads=self.n_cores, deltaW=None)
            
            # Rejuvenate the ancestor and Xi r.v.s
            if is_predicting is False and np.random.uniform(0, 1, 1) < self.rejuvenaion_prob and self.disable_ancestor_bridge is False:
                #print("REVIEW THIS...", t)
                self.a[self.N-1, t], xprime_full_path[self._Xi_slice(t-1), 0:self.K] = self.bridge.sample_joint_path_ancestor_blocked(particles=self.x[:,t-1, ], initial_weights=self.w[:, t-1], time_length=deltaT, xprime=xprime_full_path[self._t_to_tau(t),], theta=self.theta, A=A[the_slice.start:(the_slice.stop+2), :], shuffle_map=shuffle_map)    
            else:
                self.a[self.N-1, t] = np.random.choice(a=range(0, self.N), p=self.w[:, t-1], size=1)
            ...
            if is_predicting == True:
                ...
                llhood_weights = np.zeros([self.N])
                self.g.compute_loglikelihood_parallel(X=self.x[:, t, :], y=self.obs['value'][t, 0:self.K], loglikelihood=llhood_weights, n_cores=self.n_cores)
                self.w[:, t] = np.exp(llhood_weights)
...
class ParticleBridgeKernel(BridgeKernel, ParticleGibbs):
    ...
    def __init__(self, h, N, K, transitionDistribution, proposalDistribution, n_cores=1):
        """
    ...
    def _init_particles(self, time_length):
        # mesh it by timestep
        self.T = int(round(time_length/self.h, _TIME_ROUNDING_ACCURACY))
        # N by T by K
        self.x = np.empty([self.N, self.T, self.K])
    ...
    def sample_joint_path_ancestor_not_parallel(self, time_length, particles, initial_weights, xprime, theta):
        """
        ...
        """
        self._init_particles(time_length)
        self.theta = theta
        ...
        for t in range(1, self.T):
            ...
            # Propagate particles
            self.f.sample_parallel(X0=self.x[self.a[0:self.N, t], t-1,:], time_length=self.h, selection_coeffs=self.theta, seed=1, Xi=self.dummy, x_out=self.x[:, t, :], n_threads=self.n_cores)
                
            
            # Weighting
            deltaT = time_length - self.h*t
...
class BlockedParticleBridgeKernel(ParticleBridgeKernel):
    ...
    def __init__(self, h, N, K, transitionDistribution, proposalDistribution, n_cores=1):
        super().__init__(h, N, K, transitionDistribution, proposalDistribution, n_cores)
        
    
    def sample_joint_path_ancestor_blocked(self, time_length, particles, initial_weights, xprime, theta, A, shuffle_map):
        """
        particles is a N by K vector, each row represeting one of the particles over multiple dimensions
        xprime is a K by 1 vector, representing the endPoint of the Brdige, on which the enclosing PGAS algorithm is conditioned
        """
        ...
        for t in range(1, self.T):
            ...
            #self.x[:, t, :], _, _ = self.f.sample_vectorised_dumb(X0=self.x[self.a[0:self.N, t], t-1,:], A=A[(t-1):t+1, :], s=self.theta, time_length=self.h, seed=1)
            
            self.f.sample_parallel(X0=self.x[self.a[0:self.N, t], t-1,:], Xi=self.dummy, x_out=self.x[:, t, :], A=A[(t-1):t+1, :], s=self.theta, time_length=self.h, seed=1, n_threads=self.n_cores)
